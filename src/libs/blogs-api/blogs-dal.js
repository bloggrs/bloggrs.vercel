...
module.exports = {
  ...
  createBlog: async ({ name, description, logo_url, UserId, BlogCategory, BlogThemeId }) => {
    let blogcategories = await prisma.blogcategories.findFirst({ where: BlogCategory })
    if (!blogcategories) {
      blogcategories = await prisma.blogcategories.create({ data: BlogCategory })
    }
    blogcategories = { connect: { id: blogcategories.id }}
